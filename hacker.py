...
class hazirla:
    ...
    def main(self):
        try:
            ...
            print
            "File not found %s" % (self.wordlist)
            sys.exit(0)
        try:
        ...
        while not self.queue.empty():
            try:
                ...
                for rdata in answers:
                    self.lock.acquire()
                    print
                    answers.qname, ' CNAME:', rdata.target.to_text()
        ...
        try:
            ...
            for finder in self.response:
                if finder in subrespon:
                    ...
                    print
                    self.error + "--- TAKEOVER DETECTED !!! : " + subdomain
...
if __name__ == '__main__':
    try:
        ...
        if not option.domain:
            print
            "example: ./takeover.py -d host.com -w wordlist.txt  -t 10"
            sys.exit(0)

        if not option.wordlist:
            print
            "example: ./takeover.py -d host.com -w wordlist.txt"
            sys.exit(0)
